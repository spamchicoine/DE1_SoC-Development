...
void setupDataAndModels(){
    const char* filename = "final_image_bw.bmp";
    int width = 0;
    int height = 0;
...
void processTiles_weightStatinary(int numNeurons,
    ...
    int weightsPerTile = numNeurons*inputTileSize;
    ...
    #if FPGA == 1
        cl_mem weightsTileBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY, weightsPerTile * sizeof(float), NULL, &err);
        ...
        cl_mem outputTileBuffer = clCreateBuffer(context, CL_MEM_READ_WRITE, 10 * sizeof(float), NULL, &err);
        ...
        clSetKernelArg(kernel, 2, sizeof(int), (int*) &inputTileSize);
        clSetKernelArg(kernel, 3, sizeof(int), (int*) &outputNeuronsTileSize);
        clSetKernelArg(kernel, 4, sizeof(cl_mem), (void*)&outputTileBuffer);
    ...
    for (int tileIndex = 0; tileIndex < numTiles; ++tileIndex) {
        ...
        err = clEnqueueWriteBuffer(queue, weightsTileBuffer, CL_FALSE, 0, weightsPerTile * sizeof(float), temp_wts.data(), 0, NULL, NULL);
        err = clEnqueueWriteBuffer(queue, inputTileBuffer, CL_FALSE, 0, inputTileSize * sizeof(float), inputSlice.data(), 0, NULL, NULL);
        clFinish(queue);
        ...
        err = clEnqueueReadBuffer(queue, outputTileBuffer, CL_FALSE, 0, outputNeuronsTileSize * sizeof(float), (void*)outputSlice.data(), 0, NULL, NULL);
        
        clFinish(queue);

        for(int i = 0; i < numNeurons; i++){
            outputs[i] += outputSlice[i];
        }
    }
    ...
    // OpenCL kernels running on FPGA are not synchornous. You will synchronize your computations by waiting till the queue is finished by the following code    
        ...
        clReleaseMemObject(outputTileBuffer);
